...
struct compare {
    ...
    {
        size_t a { (flight_1.duration) + (flight_1.connection_times) + 3 * (flight_1.price) };
        size_t b { (flight_2.duration) + (flight_2.connection_times) + 3 * (flight_2.price) };
        return a > b;
    }
};
...
inline std::priority_queue<Flight, std::vector<Flight>, compare> gather_flights(std::string filename)
{
    ...
    std::regex pattern(R"((\d-\s)(flight_number)(:)(\w+)(\s-\s)(duration:)(\d+)(\w)(\d*)(\w*)(\s-\sconnections:)(\d)(\s-\sconnection_times:)(\d)(\w)(\d*)(\w*)(,*)(\d*)(h*)(\d*)(m*)(,*)(\d*)(h*)(\d*)(m*)(\s-\sprice:)(\d*))");
    std::smatch match;

    while (std::regex_search(txt, match, pattern)) {
        std::string flight_number { match[4].str() };
        std::istringstream iss_duration_h { match[7].str() };
        std::istringstream iss_duration_m { match[9].str() };
        ...
        flight.push_back(Flight(flight_number, (duration_h * 60) + duration_m, connections, (connection_times_1_h + connection_times_2_h + connection_times_3_h) * 60 + connection_times_1_m + connection_times_2_m + connection_times_3_m, price));
        txt = match.suffix().str();
    }

    std::priority_queue<Flight, std::vector<Flight>, compare> fight_priority_queue;
    for (const auto& i : flight)
        fight_priority_queue.push(i);
    return fight_priority_queue;
}
